    ...
    if isinstance(temperatures,list):
        for temp in temperatures:
            if temp_wait and temp is not None:
                outputs.append({'description':f'setting temperature stage to {temp} degrees and waiting\n',
                               'action':'temp',
                               'kwargs':{'temp':temp,'wait':temp_wait}})
            elif temp is not None:
                outputs.append({'description':f'setting temperature stage to {temp} degrees and continuing\n',
                               'action':'temp',
                               'kwargs':{'temp':temp,'wait':temp_wait}})
                    ...
                    if pol_mode == "sample":
                        if pol is not None and grazing_angle is not None:
                            ...
                            outputs.append({'description':f'\ncalculating a lab-frame polarization of {pol} from the sample_frame polarization \n  input of {orig_pol} and a sample angle {grazing_angle}\n',
                                               'action':'message'})
                    outputs.append(nexafs_scan_enqueue(
                                        scan_params=params,
                                        cycles=cycles,
                                        pol=pol,
                                        angle=grazing_angle,
